...
float adjustedToPressureSlope(long sensorValAdjusted);
...
const uint32_t version_number = 0x001c;                          // version number to force new calibration on major update : 0.0.1-c
...
void calibrationAsk()
{
   ...
   while(timeLeft--)
   {
      ...
      float mmHg = adjustedToPressureSlope(absAdjusted);
...
void confirm(void (*functionPass)(), void (*functionFail)())
{
   ...
   {  
      ...
      mmHg = adjustedToPressureSlope(absAdjusted);
...
void setCalibrationValues()
{
   ...
   } else
   {
     ...
     Serial.println("setting EEPROM to refValues[] and avg_slope");
...
void loop()
{
   ...
   float mmHg_interpolation = adjustedToPressure(     sensorVal);
   float mmHg_slope         = adjustedToPressureSlope(sensorVal);

   lcd.clear();
   lcd.setCursor(0, 0);
   lcd.print(mmHg_interpolation, 1);
   lcd.print("mmHg int.");
   lcd.setCursor(0, 1);
   lcd.print(mmHg_slope, 1);
   lcd.print("mmHg lin.");
